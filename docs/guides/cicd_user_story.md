...
- [x] **Automated Triggering**: The pipeline must run on every Pull Request to `dev` and `main`, and on direct pushes to `dev` and `main`.
...
flowchart TD
    ...
    subgraph CD [Continuous Deployment Job]
        J -- "dev" --> L[Deploy to Staging Folder on Websupport]
