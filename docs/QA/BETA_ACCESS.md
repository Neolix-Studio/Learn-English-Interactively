...
```text
data/migrations/20_create_beta_invites.sql
```

That file uses `CREATE TABLE IF NOT EXISTS`, so it is safe to run manually before deployment if the beta tables do not exist yet.
