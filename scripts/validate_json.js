...
function processFile(filePath) {
  ...
  try {
    const errorCountBefore = errorCount;
    ...
    reportFileError(`Failed to parse JSON: ${err.message}`);
  }
}
...
function walkDir(dir) {
  ...
  list.forEach(file => {
    ...
    const relativeParts = path.relative(dataDir, fullPath).split(path.sep);
    if (relativeParts.some(part => ignoredPathParts.has(part))) {
      return;
    }
...
if (hasErrors) {
  ...
  console.log(`\n\x1b[32mAll JSON files parsed successfully. ${validatedCount}/${parsedCount} files also matched known schema checks.\x1b[0m`);
  process.exit(0);
}
