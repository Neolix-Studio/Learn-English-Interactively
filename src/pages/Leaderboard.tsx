...
export const LeaderboardPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      if (diff <= 0) { setWeeklyCountdown('Hamarosan...'); return; }
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        if (userIndex === -1) {
            // Temporarily append the searched user if they are not in the top list
            updatedUsers.push({ username, xp, rank });
            // Sort by XP again so they appear in correct relative visual position if needed, 
            // but usually we just put them at the bottom if rank > limit.
        ...
        alert(response.message || 'Felhasználó nem található.');
      }
    } catch (e) {
      console.error(e);
    }
  ...
  const getLeagueName = (id: number) => {
    switch (id) {
        case 1: return { name: 'Bronz', color: '#CD7F32', icon: '🥉' };
        case 2: return { name: 'Ezüst', color: '#C0C0C0', icon: '🥈' };
        case 3: return { name: 'Arany', color: '#FFD700', icon: '🥇' };
        case 4: return { name: 'Gyémánt', color: '#00BFFF', icon: '💎' };
      ...
      <div className="main-stage-track">
        ...
        <div id="app" className="leaderboard-app">
          ...
          <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
              <h1 style={{ fontSize: '2.5rem', fontWeight: 900, color: 'var(--color-text-main)', marginBottom: '0.5rem' }}>Ranglista</h1>
              <p style={{ color: 'var(--color-text-muted)', fontSize: '1.1rem' }}>Versenyezz másokkal és juss feljebb a ligákban!</p>
          </div>

          {/* Timeframe Segmented Control */}
          <div className="segmented-control">
              <button 
                className={`segment-btn ${timeframe === 'weekly' ? 'active' : ''}`}
                onClick={() => setTimeframe('weekly')}
              >Heti</button>
              <button 
                className={`segment-btn ${timeframe === 'monthly' ? 'active' : ''}`}
          ...
          {timeframe === 'weekly' && (
            <div style={{ textAlign: 'center', marginBottom: '1rem' }}>
              <span style={{ fontSize: '0.82rem', color: 'var(--color-text-muted)', background: 'var(--color-bg-surface)', border: 'var(--glass-border)', borderRadius: '20px', padding: '0.35rem 1rem', display: 'inline-flex', alignItems: 'center', gap: '0.4rem' }}>
                ⏱ Heti reset: <strong style={{ color: 'var(--color-text-main)', fontVariantNumeric: 'tabular-nums' }}>{weeklyCountdown}</strong>
              </span>
            </div>
          )}
                  ...
                  <>
                      {users.length === 0 ? (
                          <div style={{ padding: '3rem', textAlign: 'center', color: 'var(--color-text-muted)', background: 'var(--color-bg-surface)', borderRadius: '16px' }}>
                              ...
                              Még senki sem szerzett pontot ebben a ligában.
                          </div>
                      ) : (
                                  ...
                                  >
                                      ...
                                      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                                          <div style={{ fontWeight: '700', fontSize: '1.1rem', color: isCurrentUser ? 'var(--color-text-main)' : 'var(--color-text-main)' }}>
                                              {user.username}
                                              {isCurrentUser && <span style={{ marginLeft: '10px', fontSize: '0.8rem', background: 'var(--color-accent-in)', color: 'white', padding: '2px 8px', borderRadius: '10px' }}>Te</span>}
                                          </div>
                                          {user.active_title && (
                      ...
                      {/* Current User appended at bottom if not in top list */}
                          ...
                          <div style={{ marginTop: '2rem' }}>
                              ...
                              >
                                  ...
                                  <div style={{ flex: 1, fontWeight: '700', fontSize: '1.1rem', color: 'var(--color-text-main)' }}>
                                      {currentUsername}
                                      <span style={{ marginLeft: '10px', fontSize: '0.8rem', background: 'var(--color-accent-in)', color: 'white', padding: '2px 8px', borderRadius: '10px' }}>Te</span>
                                  </div>
