...
export const AvatarUploadModal: React.FC<AvatarUploadModalProps> = ({ isOpen, onClose }) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        // Let's force reload or update user context avatar string if we had one.
        // The avatar is technically in the session data, which is now fetched via useQuery.
        // We can just reload the page to refresh the session data easily, or useQueryClient.
  ...
  // We fall back to standard avatar if none is in progress (though technically user doesn't have an avatar field in UserProgressData yet)
  // Let's assume the backend will handle it, or we just rely on the session reloading.
