...
export const FillBlanks: React.FC<FillBlanksProps> = ({ question, onAnswer }) => {
  ...
  // Replace the blank ____ with the selected word or keep it blank for standard view
  const displaySentence = sentenceText.replace(
    /_{3,}/, 
    selectedWord 
      ? `<span style="color: var(--color-accent-in); border-bottom: 2px solid var(--color-accent-in); padding: 0 0.5rem; font-weight: bold;">${selectedWord}</span>`
      : `<span style="color: var(--color-text-muted); border-bottom: 2px solid var(--glass-border); padding: 0 1.5rem;"></span>`
  );
      ...
      {isDialogue ? (
        <div style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '1.5rem', marginBottom: '3rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '1rem', alignSelf: 'flex-end', maxWidth: '85%', flexDirection: 'row-reverse' }}>
	        ...
	        <div 
	          dangerouslySetInnerHTML={{ __html: sanitizeHtml(displaySentence) }}
          style={{ 
            fontSize: '1.5rem', 
            marginBottom: '3rem', 
            background: 'var(--color-bg-surface)', 
          ...
          return (
            <button
              ...
              style={{
                ...
                border: isSelected ? '2px solid var(--color-accent-on)' : '2px solid var(--glass-border)',
                boxShadow: isSelected ? '0 2px 0 var(--color-accent-on)' : '0 4px 0 var(--glass-border)',
